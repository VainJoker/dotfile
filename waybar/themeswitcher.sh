    ...
    if [ ! $value == "$HOME/.dotfiles/waybar/themes/assets" ]; then
        if [ ! $value == "$themes_path" ]; then
            if [ $(find $value -maxdepth 1 -type d | wc -l) = 1 ]; then
                ...
                if [ -f $themes_path$result.dotfiles.sh ]; then
                    source $themes_path$result.dotfiles.sh
...
choice=$(echo -e "$listNames" | rofi -dmenu -replace -i .dotfiles ~/dotfiles/rofi/config-themes.rasi -no-show-icons -width 30 -p "Themes" -format i)
IFS="~"
input=$listNames2
read -ra array <<< "$input"
