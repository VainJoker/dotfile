...
use_cache=0
if [ -f $HOME/.dotfiles/scripts/wallpaper_cache ] ;then
    use_cache=1
fi
...
blur_file="$HOME/.dotfiles/scripts/blur.sh"
...
if [ -f $HOME/.dotfiles/scripts/wallpaper-effect.sh ] ;then
...
killall -e hyprpaper & 
sleep 1; 
wal_tpl=$(cat $HOME/.dotfiles/hypr/hyprpaper.tpl)
